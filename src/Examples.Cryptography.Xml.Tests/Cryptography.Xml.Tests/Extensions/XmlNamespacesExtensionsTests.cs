...
{
    [Fact]
    public void When_ToNamespaceManagerCalled_Then_ReturnsConfiguredManager()
    {
    ...
    [Fact]
    public void When_ToSerializerNamespacesCalled_Then_ReturnsConfiguredSerializer()
    {
    ...
    [Fact]
    public void When_ToSerializerNamespacesCalled_WithIncludeDefault_Then_ReturnsSerializerWithDefaultNamespaces()
    {
        var manager = new XmlNamespaceManager(new NameTable());
        manager.AddNamespace("ds", SignedXml.XmlDsigNamespaceUrl);
        manager.AddNamespace("xa", "http://uri.etsi.org/01903/v1.3.2#");
