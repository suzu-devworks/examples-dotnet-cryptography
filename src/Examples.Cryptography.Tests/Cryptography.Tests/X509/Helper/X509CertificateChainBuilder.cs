...
/// <example>
/// <code>
/// var certificates = new X509CertificateChainBuilder(new("C=JP, CN=root-ca.example"))
...
public sealed class X509CertificateChainBuilder(X500DistinguishedName rootCaSubject)
{
    ...
    /// <returns></returns>
    public X509CertificateChainBuilder AddIntermediateCA(X500DistinguishedName intermediateCaSubject)
    {
    ...
    /// <returns></returns>
    public X509CertificateChainBuilder AddEndEntity(X500DistinguishedName subject, Action<CertificateRequest> action)
    {
        ...
        int days)
    {
        ...
        var serial = new CertificateSerialNumber(200L, new Random()).ToBytes();
